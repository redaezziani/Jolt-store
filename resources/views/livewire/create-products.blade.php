<div class="w-full flex flex-col  flex-wrap justify-start items-start md:justify-between md:flex-row">
    ...
    <div class=" flex justify-start w-full md:w-1/2 items-start flex-col gap-4">
        ...
        the nessary info
        --}}
        <p class="text-slate-800 mt-5 text-lg font-bold">
            المعلومات الأساسية
        </p>
        ...
        <div class="flex mt-10 flex-col  w-full md:w-[37rem] gap-3">
            <x-label x-data x-on:click="$dispatch('color-side-bar-open')" for="color">
                لون المنتج الخاص بك
            </x-label>
            ...
            <div class="flex gap-3 flex-wrap justify-start items-center">
                <div x-data="{ color: '#ffffff' }">
                    ...
                    <button type="button" class="w-8 h-8 rounded-full" x-bind:style="{ backgroundColor: color }"
                        x-on:click="$refs.colorPicker.click()" aria-label="Select a color"></button>

                    <!-- Hidden native color input -->
                    <input type="color" x-ref="colorPicker"
                        x-on:change="color = $event.target.value; $wire.selectColor(color)" style="display: none;" />
                </div>
                <!-- Display Selected Colors -->
                @foreach ($Colors as $color)
                    @if (!empty($color))
                        <span class=" flex px-4 py-0.5 text-xs border border-slate-400/35 rounded-full"
                            style="color: {{ $color }};">
                            {{ $color }}
                        </span>
                    @endif
        ...
        <div class="flex  items-center flex-wrap md:w-[68rem] w-full justify-between md:justify-start gap-4">
            <div class="mb-4 flex flex-col items-start justify-start gap-2">
                <x-select label="اختر نوع الشحن" placeholder="اختر نوع الشحن" :options="[
                    ['name' => 'شحن مجاني', 'id' => 1],
                    ['name' => 'شحن مدفوع', 'id' => 2],
                    ['name' => 'شحن سريع', 'id' => 3],
                ]" option-label="name"
                    option-value="id" wire:model.defer="shipping" />
                <!-- End Select -->
                @error('shipping')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4 flex flex-col items-start justify-start gap-2">
                <x-select label="اختر الفئة" placeholder="اختر الفئة" :options="$categories" option-label="name"
                    option-value="id" wire:model.defer="category_id" />
                @error('category_id')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-4 mt-5 flex w-full flex-col  items-start justify-start gap-2">
            ...
            <div class=" w-full">
                <label for="dropzone-file"
                    class="flex flex-col items-center w-full md:max-w-lg  p-5  mt-2 text-center bg-white border border-slate-300  cursor-pointer dark:bg-slate-900 dark:border-slate-700 rounded-xl">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-8 h-8 text-slate-500 dark:text-slate-400">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 16.5V9.75m0 0l3 3m-3-3l-3 3M6.75 19.5a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
                    </svg>

                    <h2 class="mt-1 font-medium tracking-wide text-slate-700 dark:text-slate-200">ملف الغلاف</h2>

                    <p class="mt-2 text-xs tracking-wide text-slate-500 dark:text-slate-400">قم بتحميل أو سحب وإفلات
                        ملف
            ...
            @if ($cover_img)
                <div class="flex w-full relative">
                    <span wire:click="removeCoverImg()" class="absolute top-2 right-2 bg-white p-1 rounded-full">
                    ...
                    <img src="{{ $cover_img->temporaryUrl() }}" alt=""
                        class=" w-full aspect-[9/10] md:max-w-lg  object-cover rounded-md">
                </div>
            @endif
        </div>
        ...
        <div class="mb-4 flex flex-col w-full items-start justify-start gap-2">
            ...
            <div class=" w-full">
                <label for="prev_imgs"
                    ...
                    <h2 class="mt-1 font-medium tracking-wide text-slate-700 dark:text-slate-200">
                        ملفات الصور
                    </h2>

                    <p class="mt-2 text-xs tracking-wide text-slate-500 dark:text-slate-400">
            ...
            @if ($prev_imgs)
                <div class="flex gap-2 flex-wrap">
                    @foreach ($prev_imgs as $img)
                        <div class="flex w-full relative">
                            <span wire:click="removePrevImg({{ $loop->index }})"
    ...
    <div class="flex flex-col gap-3 w-full">
        ...
        <p class="text-slate-800 mt-10 text-lg font-bold">
            المعلومات الخصم
        </p>
        <div class="flex  items-center flex-wrap md:w-[68rem] w-full justify-between md:justify-start gap-4">
            <div class="mb-4 flex flex-col w-full md:w-fit items-start justify-start gap-2">
            ...
            <div class="mb-4 flex flex-col w-full md:w-fit items-start justify-start gap-2">
                ...
                <x-datetime-picker wire:model.live="discount_start" label="تاريخ بداية الخصم"
                    placeholder="تاريخ بداية الخصم" display-format="DD-MM-YYYY HH:mm" />
            ...
            <div class="mb-4 flex flex-col w-full md:w-fit items-start justify-start gap-2">
                ...
                <x-datetime-picker wire:model.live="discount_end" label="تاريخ نهاية الخصم"
                    placeholder="تاريخ نهاية الخصم" display-format="DD-MM-YYYY HH:mm" />
                @error('discount_end')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
